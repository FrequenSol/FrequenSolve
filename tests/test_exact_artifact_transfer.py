...
def test_slurm_fetch_outputs_keeps_required_control_postprocessing(tmp_path):
    ...
    site.fetch_artifacts = lambda job, **kwargs: calls.append("tasks")
    site.fetch_postprocess = lambda job: calls.append("postprocess")
    job = SimpleNamespace(
        _local_path=tmp_path,
        requires_postprocess=lambda: True,
    )
    assert site.fetch_outputs(job) == tmp_path / "results"
    assert calls == ["tasks", "postprocess"]
