...
def _finite_real_vector(value, *, name: str, size: Optional[int] = None) -> np.ndarray:
...
class DiagonalInverseHessian:
    ...
    def apply(self, vector: Sequence[float]) -> np.ndarray:
    ...
    def __call__(self, model: Sequence[float], vector: Sequence[float]) -> np.ndarray:
