...
class ControlSpace:
    ...
    def unpack(
        self, vector: Union[Sequence[float], np.ndarray]
    ) -> Dict[str, np.ndarray]:
...
def _normalized_frequencies(values: Sequence[Union[float, complex]]) -> list:
...
def _job_path(value: Optional[Union[str, Path]], ctx, project_relative: bool):
...
class BornControlSensitivityJob(BaseJob):
    ...
    def __init__(
        ...
        f_list: Sequence[Union[float, complex]],
        *,
...
class RTMControlSensitivityJob(BaseJob):
    ...
    def postprocess_part_outputs_exist(self) -> bool:
        ...
        return (
            all(
                self.objective_file(part).is_file()
