...
__all__ = ['Output', 'OutputManager', 'ParaviewOutput', 'ReflectivityOutput', 'WavefieldOutput']
...
class OutputManager:
   ...
   def __dict__(self) -> Dict:
      ...
      return {
         k: v for k, v in dict.items() if v
      }
   
   @classmethod
   def from_dict(cls, dict: Dict) -> None:
      receivers    = dict.get("receivers")
      paraview     = [ParaviewOutput.from_dict(pv_out)      for pv_out in dict.get("paraview",[])]
      wavefields   = [WavefieldOutput.from_dict(wf_out)     for wf_out in dict.get("wavefields",[])]
      reflectivity = [ReflectivityOutput.from_dict(ri_out)  for ri_out in dict.get("reflectivity",[])]
      return cls(write_receivers = receivers is not None,
                 paraview        = paraview, 
