...
class HPCSite(BaseSite):
      ...
      if self.config.max_duration:
         if duration:
            ...
            if duration_secs > max_duration_secs:
               warnings.warn(f"Requested duration {duration} exceeds maximum allowed duration ({self.config.max_duration});"
   ...
   def _generate_provision_script(self, nhost: int, nproc: int, duration: Optional[str] = None, **kwargs) -> str:
      name = kwargs.get("name", "FS_server")
      env = Environment(loader=FileSystemLoader('src/frequensolve/orchestrator/templates'))
      if (self.config.scheduler == "SLURM"):
   ...
   def _provision_SLURM(self, nhost: int, nproc: int, duration: Optional[str] = None, **kwargs):
      ...
      fd, script_path = tempfile.mkstemp(suffix='.sh', prefix='slurm_', dir=self.work_dir)
      ...
      if self.config.local:
         ...
         try:
            try:
               ...
               raise RuntimeError(f"Failed to connect to {self.config.hostname}: {str(e)}")
            ...
            # Submit job
            stdin, stdout, stderr = self._ssh_client.exec_command(f"sbatch {remote_path}")
            ...
            def monitor_remote_job():
               while True:
                  stdin, stdout, stderr = self._ssh_client.exec_command(f"squeue -j {job_id} -h -o %T")
                  ...
                  if "running" in status:
                     ...
                     stdin, stdout, stderr = self._ssh_client.exec_command(f"squeue -j {job_id} -h -o %N")
   ...
   def _ssh_login(self, **kwargs) -> None:
      ...
      if "hostname" not in kwargs or "username" not in kwargs:
         raise ValueError("Both 'hostname' and 'username' must be provided in kwargs")
      ...
      try:
         ...
         raise paramiko.SSHException(f"Failed to connect to {kwargs['hostname']}: {str(e)}")
   ...
   def _provision_PBS(self, nhost: int, nproc: int, duration: Optional[str] = None, **kwargs):
      """Create a PBS resource request."""
      raise NotImplementedError
   
   
   def _provision_LSF(self, nhost: int, nproc: int, duration: Optional[str] = None, **kwargs):
   ...
   def deprovision(self, **kwargs):
      ...
      if self.config.local:
         ...
         stdin, stdout, stderr = self._ssh_client.exec_command(f"scancel {self.status.job_id}")
   ...
   def get_site_status(self):
      ...
      job_id = self.status.job_id
      if job_id is None:
         return SiteStatus(status="unknown", return_code=-1, stdout="", stderr="No job ID found")
      try:
         if self.config.scheduler == "SLURM":
            if self.config.local:
         ...
         if result.returncode != 0:
            return SiteStatus(status="unknown", return_code=result.returncode, stdout=result.stdout, stderr=result.stderr)
         if re.search(r'\bR\b', result.stdout):
            return SiteStatus(status="running", return_code=0, stdout=result.stdout, stderr=result.stderr)
         elif re.search(r'\bPD\b', result.stdout):
            return SiteStatus(status="pending", return_code=0, stdout=result.stdout, stderr=result.stderr)
         else:
            return SiteStatus(status="completed", return_code=0, stdout=result.stdout, stderr=result.stderr)
      except Exception as e:
   ...
   def submit_jobs(self, jobs: List[BaseJob]) -> None:
      ...
      futures = []
      for job in jobs:
         fut = self._client.submit(job.run_mpi_flux, job.nodes, job.ranks_per_node, job.cmd)
         futures.append(fut)
      return futures
   ...
   def provision_dask_jobqueue(self, nhost: int, nproc: int, duration: Optional[str] = None, **kwargs) -> Client:
      ...
      if self.config.scheduler == "SLURM":
         cluster = SLURMCluster(
            ...
            job_script_prologue=[
               f"{self.config.mpi_wrapper} flux start --boot"
            ],
            **kwargs
         )
      elif self.config.scheduler == "PBS":
