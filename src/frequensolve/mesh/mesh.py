...
class Mesh:
    ...
    def add_vertex(self, coords: Union[List[float], np.ndarray]) -> None:
        ...
        if len(coords) != self.dimension:
            raise ValueError(
                f"Coordinate length {len(coords)} != mesh dimension {self.dimension}"
            )
        if HAS_CPP:
    ...
    def add_element(self, elem_type: str, vertices: List[int], domain: int = 1) -> None:
        ...
        if HAS_CPP:
            ...
            self._elements.append(
                Element(kind=elem_type, vertices=vertices, domain=domain)
            )
    ...
    def _write_exodus(self, file: Union[str, Path]) -> None:
        ...
        try:
            ...
            raise ImportError(
                "The exodus3 Python package must be installed to use this mesh format (e.g. `pip install exodus3`)"
            )
    ...
    def _read_gmp(self, file: Union[str, Path]) -> None:
        ...
        while i < len(lines):
            ...
            if any(
                line.endswith(s)
                for s in ["edges", "triangles", "quads", "tetras", "hexas"]
            ):
