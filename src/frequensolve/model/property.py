...
# Helper functions
# --------------------------------------------------------------------------------------------------
def _dims_compatible(dims1: List[str], 
                     dims2: List[str]) -> bool:
...
def _coords_compatible(coords1: Dict[str, ArrayLike], 
   ...
   def get(self, xarr: Optional[xr.DataArray] = None):
      ...
      if _coords_compatible(coords, self.darr.coords):
         ...
         if _dims_compatible(self.darr.dims, coords):
            ...
            return xr.DataArray(
               data=np.full(shape, self.darr.values),
               dims=dims,
               coords=coords
            )
         else:
      ...
      with h5py.File(fname, "r") as f:
         if "coords" not in f:
            if "coords" not in kwargs:
               raise ValueError("Coords not found in h5 file, must be provided via 'coords' keyword argument")
            xarr = kwargs["xarr"]
            coords = xarr.coords
            dims = xarr.dims
         else:
