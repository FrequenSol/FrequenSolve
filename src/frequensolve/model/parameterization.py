...
def _coefficient_vector(values: Sequence[float], *, minimum: int) -> np.ndarray:
...
class HatControl:
    ...
    def with_coefficients(self, coefficients: Sequence[float]) -> "HatControl":
...
class BSplineControl:
    ...
    def coordinates(self) -> np.ndarray:
        ...
        if self.degree == 0:
            return 0.5 * (self.knots[:-1] + self.knots[1:])
        return np.array(
            [
                np.mean(self.knots[i + 1 : i + self.degree + 1])
                for i in range(self.size)
            ],
            dtype=np.float64,
        )

    def with_coefficients(self, coefficients: Sequence[float]) -> "BSplineControl":
        """Return a copy with a replacement coefficient vector."""
...
class ParameterizedProperty(Property):
    ...
    def coefficients(self) -> np.ndarray:
        ...
        return self.control.coefficients
