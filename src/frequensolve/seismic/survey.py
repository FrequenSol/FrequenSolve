...
class SeismicSurvey:
        ...
        **kwargs,
        ...
        def clean_input(arg):
            if isinstance(arg, str) or isinstance(arg, Path):
                ...
                if arg.ndim == 3:
                    if dim == 2:
                        out = xr.DataArray(
                            arg,
                            dims=["iarg", "dir"],
                            coords={"iarg": np.arange(arg.shape[0]), "dir": ["x", "z"]},
                            chunk={"iarg": chunk**2},
                        )
                    elif dim == 3:
                        out = xr.DataArray(
                    ...
                    if dim == 2:
                        ...
                        out = xr.DataArray(
                            arg,
                            dims=["iarg", "type", "dir"],
                            coords={"type": ["src", "recv"], "dir": ["x", "y", "z"]},
                            chunk={"iarg": chunk**2},
                        )
                ...
                raise ValueError(
                    "'src' must be a string, Path, numpy array, or xarray DataArray"
                )
            return out
        ...
        if src_recv is None:
            if src is None or recv is None:
                raise ValueError(
                    "either a combined 'src_recv' or separate 'src' and 'recv' file or data must be provided"
                )
            ...
            urecv = [
                ds["receiver_" + k].values.astype(np.float32)
                for k in ds.receiver_.keys()
            ]
            xr.Dataset({"urecv": (("receiver",), urecv.astype(np.float32))}).to_zarr(
                "surveyDB_urecv.zarr"
            )
            xr.Dataset(
                {"src_to_urecv": (("receiver"), np.arange(len(urecv), dtype=np.uint32))}
            ...
            sources, source_indices = np.unique(
                src.data(), axis=0, return_inverse=True
            ).compute()

            # Get unique reciever list and indices
            urecv, iurecv = np.unique(
                recv.data(), axis=0, return_inverse=True
            ).compute()
            ...
            src_to_urecv = (
                xr.DataArray(src_to_urecv, dims=["source", "receiver"])
                .fillna(np.nan)
                .pad(irecv=(0, max_receivers))
            )
            ...
            xr.Dataset(
                {"src_to_urecv": (("source", "receiver"), iurecv.astype(np.uint32))}
            ).to_zarr("surveyDB_src_to_urecv.zarr")
            del urecv, iurecv, src_recv_coords, src_to_urecv

            recv_full = (
                xr.DataArray(receivers, dims=["source", "receiver", "dir"])
                .fillna(np.nan)
                .pad(irecv=(0, max_receivers))
            )
            ...
            ds = xr.Dataset(
                coords={
                    ...
                    **{
                        "source_" + k: (("source",), sources.dir[k])
                        for k in sources.dir.keys()
                    },
                    **{
                        "receiver_" + k: (("receiver",), recv_full.isel(dir=k))
                        for k in recv_full.dir.keys()
                    },
                }
            )
            ...
            ds = ds.assign_coords(
                {
                    "midpoint_x": (
                        ...
                        xr.DataArray(
                            0.5
                            * (ds.source_x + receivers.isel(receiver=ds.urecv_ind).x),
                            chunks={"source": chunk, "receiver": chunk},
                        ),
                    ),
                    **(
            ...
            # Read database in chunked format
            self.spatialDB = xr.open_zarr("surveyDB.zarr", chunks={"source": 32})
            self.unique_receivers = xr.open_zarr(
                "surveyDB_urecv.zarr", chunks={"receiver": chunk**2}
            )
            self.source_to_receivers = xr.open_zarr(
                "surveyDB_src_to_urecv.zarr",
                chunks={"source": chunk, "receiver": chunk},
            )
            ...
            raise ValueError(
                "either a combined 'src_recv' file or separate 'src' and 'recv' files must be provided"
            )

        # TODO: add units and convert if needed
        self.spatialDB.attrs = {
            "units": "",
            "description": "Spatial database for sources and receivers",
        }
    ...
    def _detect_format(self, src_file: Union[str, Path]):
        suffix2format = {"segy": "SEGY", "csv": "CSV", "txt": "plaintext", "h5": "HDF5"}
        format = suffix2format[src_file.split(".")[-1]]
        return format

    def read_source_coords(
        self, src_file: Union[str, Path], format: Optional[str] = None, **kwargs
    ):
        """
    ...
    def read_receiver_coords(
        self, recv_file: Union[str, Path], format: Optional[str] = None, **kwargs
    ):
        """
    ...
    def read_source_receiver_coords(
        self, src_recv_file: Union[str, Path], format: Optional[str] = None, **kwargs
    ):
        """
    ...
    def _plot_source_batches(self, **kwargs):
        ...
        for ic, cluster in enumerate(self.spatialDB.cluster.values):
            ...
            if self.dim == 2:
                recv_coords = np.column_stack(
                    [
                        self.spatialDB.receiver_x.values,
                        np.zeros_like(self.spatialDB.receiver_x.values),
                    ]
                )
                src_coords = np.column_stack(
                    [
                        self.spatialDB.source_x.values,
                        np.zeros_like(self.spatialDB.source_x.values),
                    ]
                )
            elif self.dim == 3:
                recv_coords = np.column_stack(
                    [self.spatialDB.receiver_x.values, self.spatialDB.receiver_y.values]
                )
                src_coords = np.column_stack(
                    [self.spatialDB.source_x.values, self.spatialDB.source_y.values]
                )
    ...
    def clip_window(self, window: List[List[float]], cluster: Optional[int] = None):
        ...
        if self.dim == 2:
            if len(window) != 2:
                raise ValueError(
                    "Window must be a list of [[xmin, xmax]] or flattened [xmin, xmax]."
                )
            xmin, xmax = window
            if isinstance(db, xr.Dataset) or isinstance(db, xr.DataArray):
                ...
                raise TypeError(
                    "The input spatialDB must be an xarray Dataset or DataArray."
                )
        elif self.dim == 3:
            if len(window) != 4:
                raise ValueError(
                    "Window must be a list of [[xmin, xmax], [ymin, ymax]] or flattened [xmin, xmax, ymin, ymax]."
                )
    ...
    def get_offsets(self, cluster: Optional[int] = None) -> xr.Dataset:
        ...
        return (
            np.abs(db)
            if self.dim == 2
            else xr.apply_ufunc(np.hypot, db.offset_x, db.offset_y, dask="parallelized")
        )

    def set_reciever_weights(
        self,
        w: Union[xr.DataArray, np.ndarray],
        ...
        **kwargs,
        ...
        db = db.assign_coords(
            ("source", "receiver"),
            xr.DataArray(
                {"w": w, "mask": self._mask_missing_recievers(cluster)},
                chunks={"source": chunk, "receiver": chunk},
            ),
        )
        return db
    ...
    def _cluster_sources(self, cluster_size: int = 64):
        ...
        self.spatialDB = self.spatialDB.assign_coords(
            cluster=("source", cluster_labels)
        )
    ...
    def _balance_group_sizes(
        self, points: np.ndarray, cluster_labels: np.ndarray, n_clusters: int
    ):
    ...
    def _get_unique_receivers(self, cluster: Optional[int] = None):
        ...
        if self.recievers_independent:
            ...
            if cluster is None:
                ...
                recv_indices = np.unique(
                    self.source_to_receivers.isel(source=db.source.values).values
                ).compute()
            return recv_indices
