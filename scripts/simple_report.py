...
try:
    ...
    def add_model_section(section):
        def gen_figures():
            ...
            pv = ParaviewManager(input_file=input_file, fontsize=32)
    ...
    for field in acq.list_fields():
        for source in acq.list_sources():
            ...
            if "field" in field:
                ...
                plot_gather(shot, A=0.2, cmap="gray", Tf=3.2, units="km", save=file)

                section.add_figure(
                    Figure(
                        title=f"Shot {source} -- {field}".replace("_", ""), image=file
                    )
                )
